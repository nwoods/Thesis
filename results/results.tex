...
The search procedure finds a modest excess of VBS signal, at the level of 2.7 standard deviations over the null hypothesis of the SM without VBS {\ZZ} production, compared to the expected significance of 1.6 standard deviations.
...
The {\ZZ} invariant mass is shown in Fig.~\ref{fig:aTGC} for all events in the on-shell selection, with example distributions shown for potential scenarios with nonzero aTGCs.
The limit setting procedure described in Section~\ref{sec:aGCSearch} are applied to each aTGC parameter, with all other couplings fixed to their SM values, to yield one-dimensional 95\% CL limits,
...
\begin{figure}[htbp]
  \begin{center}
    \includegraphics[width=0.48\textwidth]{results/limits2D_f4.pdf}
    \includegraphics[width=0.48\textwidth]{results/limits2D_f5.pdf}
...
The observable used for limit setting is still $m_{\ZZ}$, which is shown for these events along with example distributions for scenarios with nonzero aQGCs, in Fig.~\ref{fig:aQGC}.
...
\begin{equation}\label{eq:aQGC1D}
  ...
  \end{aligned}
\end{equation}
and represent the most stringent constraints to date on all five parameters.
