...
For symmetric beams which each have $n_b$ colliding gaussian bunches of intensity (i.e.\ number of protons in the bunch) $N_b$, orbiting the ring with frequency $f_\textit{rev}$ and relativistic factor $\gamma=E_p/m_p$, the instantaneous luminosity is given by
\begin{equation}
...
Event sizes on disk of 1--$2\unit{MB}$ mean that the raw data generation rate of CMS is around $40\unit{TB}/\unit{s}$, which could potentially be several PB/s with full reconstruction, substantially more than can be read out, stored or analyzed with current technology.
